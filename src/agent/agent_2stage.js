...
import { TwoStagePipeline } from './two_stage_pipeline.js';
...
export class Agent {
    async start(load_mem=false, init_message=null, count_id=0) {
        ...
        this.two_stage = new TwoStagePipeline(this, settings.profile);
        this.pipeline_state = {
            enabled: false,
            goal: null,
            artifact: null,
    ...
    if (!settings.task) return null;
        ...
        async advanceTwoStagePipeline() {
            try {
                if (!this.pipeline_state?.enabled || !this.pipeline_state?.artifact) return null;

                const result = await this.two_stage.runStageB(
                    this.pipeline_state.artifact,
                    this.pipeline_state.last_action_result
                );

                this.pipeline_state.last_stage_b = result.stageB;
                this.pipeline_state.artifact = this.two_stage.applyStageBResult(
                    this.pipeline_state.artifact,
                    result.stageB
                );
                ...
                await this.history.add(
                    'system',
                    `Two-stage Stage B result:\n${JSON.stringify(result.stageB)}\nUpdated artifact:\n${JSON.stringify(this.pipeline_state.artifact)}`
                );
                this.history.save();

                return result;
            } catch (err) {
                console.error(`[${this.name}] Failed to advance two-stage pipeline:`, err);
                await this.history.add('system', `Two-stage Stage B failed: ${String(err)}`);
                this.history.save();
                return null;
            }
        }
    // async runTwoStagePipeline(goal) {
    // try {
    //     console.log(`[${this.name}] Running two-stage pipeline for goal:`, goal);

    //     const result = await this.two_stage.run(goal);

    //     console.log(`[${this.name}] Stage A output:`, JSON.stringify(result.stageA, null, 2));
    //     console.log(`[${this.name}] State summary:`, JSON.stringify(result.state, null, 2));
    //     console.log(`[${this.name}] Stage B output:`, JSON.stringify(result.stageB, null, 2));

    //     await this.history.add('system',
    //         `Two-stage pipeline result.\n` +
    //         `Stage A: ${JSON.stringify(result.stageA)}\n` +
    //         `Stage B: ${JSON.stringify(result.stageB)}`
    //     );
    //     this.history.save();

    //     return result;
    // } catch (err) {
    //     console.error(`[${this.name}] Two-stage pipeline failed:`, err);
    //     await this.history.add('system', `Two-stage pipeline failed: ${String(err)}`);
    //     this.history.save();
    //     return null;
    //     }
    // }
